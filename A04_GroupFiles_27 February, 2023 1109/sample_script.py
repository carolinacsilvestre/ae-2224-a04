...
def plot_air_parcel_trajectories_map(Air_Parcel_ID):
    if attila_switch == True:
        ...
        plot2 = ax.scatter(plon[:, parcel2], plat[:, parcel2], s=20, marker='o', color='green',
                           zorder=2)
...
def plot_emission_point_trajectories_map(Emission_point):
    if attila_switch == True:
        ...
        parcel_start = Emission_point * 50
        parcel_end = parcel_start + 50
        ...
        # Plot all Lagrangian air parcels in an emission point
        for i in range(parcel_start, parcel_end):
            plot2 = ax.scatter(plon[:, i], plat[:, i], s=2, marker='o', color='green',
                               zorder=2)
            ax.scatter(plon[0, i], plat[0, i], s=140, marker='$S$', color='red',
                       zorder=2)
            ax.scatter(plon[-1, i], plat[-1, i], s=140, marker='$F$', color='red',
                       zorder=2)
